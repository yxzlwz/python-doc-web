...
字典由**键值对（key-value pairs）**构成。格式：`{a1: b1, a2: b2, ...}`，其中 a 表示键（key），b 表示值（value），一个 a 与其对应的 b 共同组成一个键值对。

当然，键值对的数据类型不仅仅局限于字符串。键的类型可以是 `int` `float` `str` `bool` 等**可哈希**且**固定**的类型，值可以是任意的或 Python 对象。

::: info 名词解释
哈希：简单来说，就是一种将任意长度的消息压缩到某一固定长度的消息摘要的函数。
:::

::: info
以上提到的四种类型再加上[元组 tuple](/3wxob8dx/) 几乎是你可以接触到的 Python 内置的所有符合键要求的类型（即“可哈希对象”），更多的类型需要自行去定义。
...
增加和修改的格式一样，直接使用形如 `dict[key] = value` 的语句即可；对于已经存在的键值对，也可以把值直接当作变量操作，例如 `dict[key] += 1`。
