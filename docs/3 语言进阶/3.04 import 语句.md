...
如果你仔细研究过其他编程语言，或者是有一定的英语功底，在看到这个词的那一刻应该或多或少已经可以猜到这个语句是干嘛的了——没错，`import` 语句是用来**引入模块**的。就像 C++ 的 `import`、JS 的 `import`一样！

## 具体用法

一般来说，我们会将 `import` 语句放在 Python 文件的最开始，就像这样：
