...
我（[异想之旅](https://www.github.com/yxzlwz)）删除了这部分的所有内容，因为我认为这些使用`pathlib`中介绍的方式操作会更加优雅。

## 运行终端命令

我（还是我[异想之旅](https://www.github.com/yxzlwz)）也删除了这部分内容，因为我觉得使用`subprocess`将会更加优雅（事实上`os.popen`就是对`subprocess`的一个不太成功的封装）。
